...
// ApiAsyncTaskInfo 必须按归属用户过滤:自增 id 可枚举,任意登录用户
...
func TestApiAsyncTaskInfo_ScopesToOwner(t *testing.T) {
	...
	if info, err := ps.ApiAsyncTaskInfo(context.Background(), 81, "alice"); err == nil {
		t.Errorf("IDOR: alice fetched bob's task id=81 (user_name=%q)", info.UserName)
	}
	// 正常:alice 查自己的 id=80 → 拿到。
	info, err := ps.ApiAsyncTaskInfo(context.Background(), 80, "alice")
	if err != nil {
