...
run_file_type_scope() {
    if [ "${#shell_scope_paths[@]}" -gt 0 ]; then
        printf '\n==> scoped gate: ShellCheck changed shell files\n'
        "$ROOT/scripts/shellcheck_runner.sh" "${shell_scope_paths[@]}"
        printf '\n==> scoped gate: shfmt changed shell files\n'
        "$ROOT/scripts/shfmt_runner.sh" -d -i 4 "${shell_scope_paths[@]}"
    else
        skip "ShellCheck and shfmt (no changed shell files)"
    fi

    if [ "${#structured_scope_paths[@]}" -gt 0 ]; then
        local files_from0
        local status
        files_from0="$(mktemp)"
        printf '%s\0' "${structured_scope_paths[@]}" >"$files_from0"
        if python3 scripts/check_repository_files.py \
            --check --files-from0 "$files_from0"; then
            rm -f "$files_from0"
        else
            status=$?
            rm -f "$files_from0"
        ...
        skip "repository file checker (no changed Markdown/YAML/JSON files)"
    fi

    if [ "${#workflow_scope_paths[@]}" -gt 0 ]; then
        printf '\n==> scoped gate: actionlint changed workflows\n'
        "$ROOT/scripts/actionlint_runner.sh" -no-color \
            "${workflow_scope_paths[@]}"
    else
        skip "actionlint (no changed workflows)"
    fi
}
...
run_go_scope() {
    ...
    if [ "$go_foundation" -eq 1 ] || [ "$go_deleted" -eq 1 ]; then
        ...
        if [ "$go_race_changed" -eq 1 ]; then
            PHYTOMNI_RUN_RACE=1 run_group server-runtime
        else
...
run_go_race_tool() {
    local package="$1"
    printf '\n==> scoped gate: go test -race %s\n' "$package"
    (cd "$ROOT/apps/server" && go test -race "$package")
}
...
for path in "${changed_paths[@]}"; do
    case "$path" in
    static-analysis-exemptions.toml | docs/development/static-analysis-exemptions.md | Makefile | *.toml | *.yaml | *.yml | .github/workflows/* | .githooks/* | scripts/check_static_analysis_exemptions.py | scripts/static_analysis/* | scripts/gates/* | scripts/run_gate_group.sh | scripts/validate_web_local.sh | scripts/scoped_gate.sh | scripts/scan_secrets.py | scripts/check_*.py | scripts/*_runner.sh | scripts/quality_runner_common.sh | scripts/install_git_hooks.sh | scripts/tests/test_gate_contract.py | scripts/tests/test_scoped_gate.py | scripts/tests/test_makefile_contract.py | scripts/tests/test_hook_contract.py | scripts/tests/test_quality_runners.py | apps/web/package.json | apps/web/package-lock.json | apps/web/tsconfig*.json | apps/web/.eslintrc.cjs | apps/web/eslint.config.mjs | apps/web/vite.config.mts | apps/web/vitest.config.mts | apps/web/.prettierrc.cjs | apps/server/go.mod | apps/server/go.sum | apps/server/config/*)
        policy_changed=1
        ;;
    esac
    case "$path" in
    ...
    if [ -e "$ROOT/$path" ]; then
        case "$path" in
        *.sh | .githooks/*) shell_scope_paths+=("$path") ;;
        .github/workflows/*) workflow_scope_paths+=("$path") ;;
        *.md | *.markdown | *.json | *.yaml | *.yml)
            structured_scope_paths+=("$path")
            ;;
        esac
    fi
    case "$path" in
    apps/server/db/* | apps/server/log/* | apps/server/graceful/* | apps/server/cron/* | apps/server/cache/*)
        go_race_changed=1
        ;;
    esac
done
...
PHYTOMNI_HYGIENE_SCOPE=changed run_group hygiene

if [ "$mode" != "precommit" ]; then
    printf '\n==> scoped gate: commit-range secret scan\n'
    python3 scripts/scan_secrets.py --range "$base..HEAD"
fi
