...
// task10RunRecordSequenceServer serves successive run snapshots so the tests
// can distinguish status-only polling from revision-aware reconciliation.
func task10RunRecordSequenceServer(t *testing.T, bodies ...string) {
	t.Helper()
	if len(bodies) == 0 {
...
func task10Projection(t *testing.T, username string, id int64) BotRunProjection {
	t.Helper()
...
func TestSyncBotRunsRevisionAdvancesWhileStatusStaysRunning(t *testing.T) {
	...
	task10RunRecordSequenceServer(t,
	...
	projection := task10Projection(t, "alice", 155)
...
func TestSyncBotRunsKeepsIntermediateOnOptionalFailure(t *testing.T) {
	...
	task10RunRecordSequenceServer(t, `{"run_id":"run-opt","agent":"deep_genome","status":"running","result":{"report_stage":"intermediate","report_completeness":"partial","report_revision":2,"intermediate_report":"# Optional report","final_report":"","degraded":true,"degraded_reason":"optional analysis unavailable","failures":[{"work_item_key":"brief_gene","status":"failed","message":"optional analysis failed"}]}}`)
	...
	projection := task10Projection(t, "alice", 156)
...
func TestSyncBotRunsKeepsIntermediateOnFinalFailure(t *testing.T) {
	...
	task10RunRecordSequenceServer(t, `{"run_id":"run-fail","agent":"deep_genome","status":"failed","result":{"report_stage":"final","report_completeness":"partial","report_revision":4,"intermediate_report":"# Partial report","final_report":"","failures":[{"work_item_key":"synthesis","status":"failed","message":"final synthesis failed"}]}}`)
	...
	projection := task10Projection(t, "alice", 157)
...
func TestSyncBotRunsPrefersFinalReportAndIgnoresEmptyArtifacts(t *testing.T) {
	...
	task10RunRecordSequenceServer(t, `{"run_id":"run-final","agent":"deep_genome","status":"succeeded","result":{"report_stage":"final","report_completeness":"complete","report_revision":5,"intermediate_report":"# Intermediate","final_report":"# Final","artifacts":[]}}`)
	...
	projection := task10Projection(t, "alice", 158)
...
func TestSyncBotRunsOlderRevisionCannotClobberVisibleReport(t *testing.T) {
	...
	projection := task10Projection(t, "alice", 159)
