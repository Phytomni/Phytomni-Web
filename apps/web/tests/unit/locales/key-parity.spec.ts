...
const walkSrc = (dir: string): string[] => {
  ...
  for (const ent of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (ent.isDirectory()) {
      ...
      /\.(ts|vue)$/.test(ent.name) &&
      !/\.(spec|test)\.ts$/.test(ent.name)
    ) {
      out.push(full);
    }
  }
...
describe("i18n key parity", () => {
  ...
  it("detects drift (negative control)", () => {
    ...
    const onlyEn = leafKeys(mockEn).filter(
      (k) => !new Set(leafKeys(mockZh)).has(k)
    );
...
describe("i18n reference resolvability", () => {
  ...
  for (const [key, file] of refs) {
    it(`"${key}" resolves in both bundles`, () => {
      expect(
        hasKey(zhCN, key),
        `${key} missing in zh-CN (used in ${file})`
      ).toBe(true);
      expect(
        hasKey(enUS, key),
        `${key} missing in en-US (used in ${file})`
      ).toBe(true);
    });
  }
});
