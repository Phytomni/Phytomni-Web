...
The current `vitest.config.mts` `coverage.include` lists 3 fully-tested files: `src/utils/auth-redirect.ts` / `src/utils/auth.ts` / `src/components/LangSwitch.vue`. Thresholds: lines/functions/statements 80%, branches 75%.
...
4. If the new test covers a new source file, expand `vitest.config.mts` `coverage.include`
