<!-- agents-compose: DO NOT EDIT — regenerate via 1.phytomni/.claude/shared/compose_agents.py -->
<!-- agents-compose-fingerprint: e019c343401f6c5dd0c13c35be37adfb5c63453cc29a7700fd87efe97ba9bd35 -->

# CLAUDE.md

This file provides guidance to AI coding agents (Claude Code, Codex, Cursor, etc.) when working in Phytomni-Web. `CLAUDE.md` is a symlink to `AGENTS.md`. The SHARED block is generated from `1.phytomni/.claude/shared/AGENTS-COMMON.md`; edit that file (and `AGENTS.local.md`) then run `compose_agents.py` — do not hand-edit the SHARED/LOCAL markers.

<!-- BEGIN SHARED -->
## AI Coding Agent Rules

Behavioral rules for Codex, Claude Code, Cursor, and other coding agents.
Repo-specific stack, architecture, and commands live in the LOCAL section below
(and in that repo's docs). This SHARED block is the cross-repo process baseline
for Phytomni-Web and Phytomni-Bot.
...
comments, or formatting. Match existing style. Every changed line should trace
to the user's request. Remove imports/variables/functions that *your* changes
made unused; don't delete pre-existing dead code unless asked.

**Goal-Driven Execution** — Define a verifiable success criterion before
starting. For multi-step tasks, state a brief plan with per-step checks
(e.g. `1. step → verify: check`). Loop until the criterion is met.

### Basic Principles And Workflow

- Read relevant code, tests, and docs before editing.
- Confirm requirements; ask instead of guessing business rules.
- Prefer the smallest viable change. Avoid unrelated refactors and formatting.
- Do not generate a large diff in one pass when a smaller sequence works.
- Do not delete, skip, or weaken tests to make checks pass.
- Do not hide failures, swallow exceptions, or fabricate results.
- For bug fixes, add a reproducing test first when a test runner exists.
- For new behavior, cover happy and failure paths.
- Classify risk before work starts; default to L2 when uncertain.
- For L2/L3 work: present a plan, expected files, and test strategy, then wait
  for human confirmation before editing.
- Run the checks appropriate to the change. If a check cannot run, say why —
  never pretend it passed.
- Final replies for changes must include the Change Report template below.
  L2/L3 changes must also include Audit Notes.

#### Repo note — gate commands

Per-repo LOCAL sections document the exact gate commands (`validate_web_local.sh`,
`make full`, etc.). Do not invent gate names from memory; read LOCAL.

### Risk Levels

Judge the risk level before starting a task.

| Level | Scope | Action |
|---|---|---|
| **L0: Low** | Docs, comments, demos, non-production scripts, local UI copy | Edit directly; keep the diff clear |
| **L1: Medium-Low** | Internal tools, non-core features, local bug fixes, features not touching sensitive data | Add or update tests |
| **L2: High** | Auth/permissions, user data R/W, DB schema, background jobs, external APIs, payments, file uploads, security logic, production config | Present plan → wait for human confirmation |
| **L3: Extremely High** | Data migrations, data deletion, encryption/keys/tokens, production deploy scripts, CI/CD release flows, infra config, permission-model rewrites | Do NOT modify without explicit human approval |

If the risk level is unclear, default to L2.

### Hard Stops Without Approval

Without explicit human approval, do not:

- Modify `.env*`, keys, tokens, certificates, production configuration,
  database migrations, auth/authorization, payment, encryption, or audit-log logic.
- Read or output real secrets or private user data.
- Run destructive commands such as `rm -rf`, `git reset --hard`, `git clean -fd`,
  `git push --force`, `drop database`, `truncate table`, `kubectl delete`, or
  `terraform apply` / `terraform destroy`.
- Add dependencies without explaining need, alternatives, license, maintenance,
  security, size, and performance impact.
- Perform large refactors without a design explanation.
- Delete tests, skip tests, or weaken assertions.
- Hide real issues just to pass lint or CI.
- Automatically commit, push, or merge unless the user explicitly asks.
- Write to, delete from, or alter gitignored high-risk trees you did not create
  in this task (examples appear in each repo's LOCAL notes) without investigating
  provenance first.

### Security, Data, And Integrations

- Do not print, log, or commit secrets. Do not write sensitive data to logs.
- Validate all user input. Keep permission checks server-side.
- Never trust frontend-supplied `user_id`, `role`, or `tenant_id`.
- In multi-tenant paths, verify users can access only their own tenant's data.
- For uploads, validate type, size, path, and permission.
- Error messages must not reveal internals or sensitive data.
- For data writes, consider idempotency, transactions, and concurrency.
- For external APIs, handle timeouts, retries, rate limits, and failures.
- For database work, explain schema/data impact, rollback, backfill, locks,
  production performance, and consistency risk. Do not create or modify
  production migrations without confirmation.
- Keep credentials, SSH keys, object-storage keys, and database URLs out of new
  files, commits, and example commands. Prefer scrubbed `.env.example` /
  `*.yml.example` patterns from LOCAL.

### Verification And Change Report

Run the lint, typecheck, unit/integration tests, and build checks that LOCAL
documents for the files you touched. Prefer the repo's single-command gate when
one exists. If a check cannot run, explain why.

Final replies for code changes must include:
...
For L2/L3 changes, also include:
...
**English-only.** Subject line and every body bullet must be English, even when
the working conversation is Chinese. Writing the commit is the language-switch
boundary.

Subject: `<emoji> Category: Capitalized imperative`
(e.g. `✨ Add`, `🐛 Fix`, `📝 Docs`, `🧪 Tests`, `♻️ Reorg`).

Body: required `- ` bullets only; blank line between subject and body; first
bullet narrates the gap/why, then what the change does. No prose preamble.
No `Co-Authored-By` trailer. No local plan/phase numbers in subject, body, or
PR text visible to collaborators.

**Never reference internal session staging concepts** in commits or PR bodies
(e.g. "Phase 1", "Stage N", "PR1", "Step 1.2", "this phase", "the plan",
"subsequent steps will…"). Describe the technical change instead:

| Bad | Good |
|---|---|
| "Step 4.3 of handler refactor" | "Route analyst handler through shared kwargs builders" |
| "Phase 1 of live e2e run" | "Resolve pytest-asyncio deadlock in ChatAgent fixture lifecycle" |
| "Continuing Step 4.2's reduction" | "Drop inline kwargs spread from the analyst handler" |

Before drafting, run `git log -3 --format=%B` in the target repo and match that
style. When dispatching a subagent that commits, require a bullet body — never
a subject-only `-m`.

PRs should describe the changed area, list verification commands, link issues,
and attach screenshots for UI changes.

## Production Sync Policy (framing)

Cross-repo coordination for Phytomni-Web ↔ Phytomni-Bot ↔ the local production
rsync mirror under `1.phytomni/project/`. These AGENTS files are gitignored, so
this section is local working-set guidance, not a public upstream commitment.

1. **24–48h backport window:** every production hot-fix that belongs upstream
   MUST result in a `[prod-backport]` PR against the matching repo within
   24–48 hours. Drift is technical debt.
2. **PR title:** `[prod-backport] <area>: <one-line summary> (prod YYYY-MM-DD)`.
   English imperative. Commit subjects still follow the emoji + Category style.
3. **PR body:** list the production file backup name (`.bak_<MMDD>` style),
   validation commands run, any schema/config changes, and local gate output
   (exact commands are in each repo's LOCAL footnotes).
4. **Schema / persistence:** document cross-repo impact in the PR body; ops
   execute production `ALTER TABLE` / cutovers. Do not invent schema ownership
   — read LOCAL and active handoff docs.
5. **Never bring secrets into PRs:** no production `.env`, `config/app.yml`,
   `auth_config.json`, or logs. Use scrubbed examples.
6. **Backport PRs may skip RFC** because they chase production reality, but the
   full local gate must still pass.
7. **Local plan terms stay local:** never reference plan codes such as
   `Phase D`, `Step B+.5`, `R2`, or `C1.4` in commit messages or PR
   descriptions; use technical language or relative references.

References (paths relative to `1.phytomni/`):

- Local backport plan: `.claude/plans/production-backport.md`
- Cross-repo reconciliation notes: `.claude/reference/reconciliation-notes.md`
- Architectural overview: `.claude/reference/项目总览.md`
<!-- END SHARED -->
